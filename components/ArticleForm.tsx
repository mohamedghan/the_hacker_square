...
const ArticleForm = () => {
  ...
  const handleFormSubmit = async (prevState: any, formData: FormData) => {
    try {
      ...
      const result = await createArticle(prevState, formData, write);
  ...
  return (
    <form action={formAction} className="article-form">
      ...
      <div data-color-mode="light">
        ...
        <MDEditor
          value={write}
          onChange={(value) => setWrite(value as string)}
          id="content"
          preview="edit"
          height={300}
        ...
        {errors.content && <p className="article-form_error">{errors.content}</p>}
      </div>
